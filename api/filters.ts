...
async function handle(req: Request): Promise<Response> {
  ...
  if (req.method === "GET") {
    ...
    const { rows } = await dbExecute(
      "SELECT id, name, query FROM saved_filters WHERE user_id = ? ORDER BY created_at DESC",
      [userId],
    );
  ...
  if (req.method === "POST") {
    ...
    await dbExecute(
      "INSERT INTO saved_filters (id, user_id, name, query) VALUES (?, ?, ?, ?)",
      [id, user_id, name, JSON.stringify(query)],
    );
  ...
  if (req.method === "DELETE") {
    ...
    await dbExecute("DELETE FROM saved_filters WHERE id = ? AND user_id = ?", [id, userId]);
    return json({ ok: true });
  }
